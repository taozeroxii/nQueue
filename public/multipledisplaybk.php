...
<body class="bg-slate-900 min-h-screen text-white overflow-hidden">
    ...
    <script>
        // State
        // URL Params Logic for Kiosk Persistence
        const urlParams = new URLSearchParams(window.location.search);

        let currentDeptFilter = urlParams.get('dept') || localStorage.getItem('dept_filter') || '';
        let currentRoomFilter = urlParams.get('room') || localStorage.getItem('room_filter') || '';
        let ttsPrefix = urlParams.get('prefix') || localStorage.getItem('tts_prefix') || 'ขอเชิญหมายเลข';
        let ttsMiddle = urlParams.get('middle') || localStorage.getItem('tts_middle') || 'ที่ห้องตรวจ';
        let ttsRepeat = parseInt(urlParams.get('repeat') || localStorage.getItem('tts_repeat')) || 1;

        // Connection Settings
        let apiBase = urlParams.get('api') || localStorage.getItem('api_base') || ''; // Default empty = relative
        let wsUrl = urlParams.get('ws') || localStorage.getItem('ws_url') || 'ws://localhost:8765';
        ...
        async function saveSettings() {
            ...
            if (wsChanged) {
                ...
                // connectWS will be called automatically by onclose or we can force it
        ...
        async function fetchQueue() {
            try {
                let url = getApiUrl('api/queue_data.php?limit=50'); // Fetch more so we can carousel
                if (currentDeptFilter) {
        ...
        function tryAutoUnlock() {
            ...
            // Try to speak a silent char to trigger unlock
            ...
            // If it succeeds, onstart fires. If blocked, it won't fire until user interaction.
            window.speechSynthesis.speak(u);
        }

        // Try to unlock on load (works in Kiosk mode or if specific flags are set)
        window.addEventListener('load', () => {
            // If Kiosk Mode detected (via URL or heuristic), allow "silent fail" or hide overlay immediately
            // so it doesn't block the view.
            if (urlParams.has('kiosk') || urlParams.has('dept')) {
                const overlay = document.getElementById('sound-overlay');
                if (overlay) overlay.classList.add('hidden');
            }
        ...
        function speakQueue(item) {
            ...
            const text = `${currentPrefix} ${item.oqueue || item.vn} ${currentMiddle} ${item.room_number}ค่ะ`;
        ...
        function playGoogleTTS(text) {
            return new Promise((resolve, reject) => {
                const audio = new Audio(`test.php?text=${encodeURIComponent(text)}&lang=th`);
        ...
        function populateVoiceList() {
            ...
            inputVoice.innerHTML = '<option value="">Default (Auto)</option>' +
                '<option value="google_api" ' + ('google_api' === saved ? 'selected' : '') + '>Google TTS (Server)</option>' +
